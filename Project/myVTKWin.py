...
class myVtkWin(vtk.vtkRenderer):
  ...
  def addSurf(self, verts, faces, color=[1., 0., 0.], opacity=1.,
    ...
    obj = surfActor(np.asarray(verts), np.asarray(faces), color=color, opacity=opacity,
                    colortable=colortable, coloridx=coloridx)
    self.objlist.append(obj)
    actor = obj.actor
...
def cylinder(vert1, vert2, rad=1.0, numcirc=16):
  ...
  for i in range(numcirc):
    faces[i + numcirc + 2 * (numcirc - 2), :] = np.array(
      [(i + 1) % numcirc, (i + 1) % numcirc + numcirc, i + numcirc, ])
...
def demoSurfaceEdgesAndColors():
  ...
  win.addSurf(sverts, sfaces, ambient=0.9, opacity=1, edgeColor=[0., 0., 0.], colortable=colortable,
              coloridx=coloridx)
...
def brainPointPick():
  ...
  class printPickWin(myVtkWin):
    def keypress_callback(self, obj, ev):
      ...
      print(
        f'Picked point coordinate: {worldPosition[0]:.2f} {worldPosition[1]:.2f} {worldPosition[2]:.2f}')
...
def demoSurfaceFromNRRD():
  import nrrd
  import nibabel as nib# % Class to create interactive 3D VTK render window
# % EECE 8396: Medical Image Segmentation
# % Spring 2024
# % Author: Prof. Jack Noble; jack.noble@vanderbilt.edu
#
...
def connectedComponents(self):
  ...
  edge_hash = edges[:, 0] * max(self.faces.flatten()) + edges[:, 1]  # Unique identifier for edges
  ...
  for i in range(n_components):
    ...
    # Filter faces where all three vertices belong to the current component
    ...
    if component_faces:
      ...
      component_faces = np.array(
        [[new_indices_map[vertex] for vertex in face] for face in component_faces])
...
if __name__ == "__main__":
  ...
  filePath = ('/Users/leonslaptop/Desktop/2024 Spring/ECE 3892/data/0522c0001/img.nrrd')
  isolevel = 700
  print("Loading and processing structure...")
  s = loadAndProcessStructure(filePath, isolevel)
  print(f"Surface loaded with {len(s.verts)} vertices and {len(s.faces)} faces.")
