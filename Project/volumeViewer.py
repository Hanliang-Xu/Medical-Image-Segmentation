...
class volumeViewer(myVtkWin):
    ...
    def onKeyPress(self,event):# for matplotlib window
        ...
        if event.key in ['up', 'a']:
            self.slc[self.focus] = min(self.slc[self.focus] + 1, np.shape(self.img)[self.focus] - 1)
        ...
        # Trigger an update to refresh the display with new slice, level, or contrast
        self.update()
